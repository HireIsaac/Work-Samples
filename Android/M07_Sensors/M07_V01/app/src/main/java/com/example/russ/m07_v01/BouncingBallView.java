...
public class BouncingBallView extends View implements SensorEventListener {
    ...
    private ArrayList<Cursor> cursors = new ArrayList<Cursor>(); // list of Balls
    private Cursor cursor_1;  // use this to reference first ball in arraylist
    private Box box;
...
//        cursors.add(new Cursor(Color.GREEN));
////        cursor_1 = cursors.get(0);  // points ball_1 to the first; (zero-ith) element of list


        cursor_1 = new Cursor(Color.GREEN);

        Log.w("BouncingBallLog", "Just added a bouncing ball");

        //ball_2 = new Ball(Color.CYAN);
        //cursors.add(new Cursor(Color.CYAN));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        cursor_1.moveWithCollisionDetection(box);
        cursor_1.draw(canvas);
        for (Cursor c : cursors) {
            c.draw(canvas);  //draw each ball in the list
            //c.moveWithCollisionDetection(box);  // Update the position of the ball
        }
        if (cursors.size() < 2000) {
            cursors.add(new Cursor(Color.CYAN, (float)cursor_1.getX(), (float)cursor_1.getY(), 0, 0));
        } else {
            cursors.clear();
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_MOVE:
                ...
                cursor_1.speedX += deltaX * scalingFactor;
                cursor_1.speedY += deltaY * scalingFactor;
                //Log.w("BouncingBallLog", " Xspeed=" + ball_1.speedX + " Yspeed=" + ball_1.speedY);
                Log.w("BouncingBallLog", "x,y= " + previousX + " ," + previousY + "  Xdiff=" + deltaX + " Ydiff=" + deltaY + "count: " + cursors.size());
                //cursor_1.cycleColor();
                for (Cursor c : cursors) {
                    c.cycleColor();
                }
